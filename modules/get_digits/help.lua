...
jester.help.get_digits.actions.get_digits.params = {
  ...
  timeout = [[Number of seconds to wait for max_digits before trying to validate.  Default is 3.]],
  terminators = [[A string of keys that the user can use to end the collection before the timeout.  Multiple values keys can be used, eg. '*' or '*#'.  To accept no terminators, pass an empty string.  Default is '#'.]],
  ...
  bad_input = [[An audio file to play to the user when input validation fails.  Default is 'ivr/ivr-that_was_an_invalid_entry.wav'.]],
  digits_regex = [[A regular expression to use for validating the user input.  If the user input does not match the expression, then validation fails.  The regex is in the same form as the regular expressions used in the FreeSWITCH dialplan.  Default is '\\d+', which matches one or more digits.  Note: If you need to match the * key in the regex, you will have to escape it twice, as in '\\d+|\\*'.]],
  storage_key = [[The key to store the collected digits under in the 'get_digits' storage area.  Default is 'digits'.]],
}
